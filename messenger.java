...
public class messenger {
		...
		public static void main(String[] args) throws FileNotFoundException{
			...
			while(sc.hasNextLine()){
				...
				String[] tokens = command.split("_"); //split string 1st time by "_"
				System.out.print("\n");
				...
				if(tokens[0].equals("add")){
					...
					if(split_1[0].equals("user")){//add_user(u_id,u_name)
						...
						//System.out.println(g_id > 0 &&  !g_name.substring(0,1).matches("\\s") && !g_name.substring(0,1).matches("\\d"));
				...
				if(tokens[0].equals("send")){
					...
					//String text = tokens[1].split("\\(")[1].split(",")[2].split("\\)")[0].trim();
